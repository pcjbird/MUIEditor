...
namespace DuiLib
{
	...
	// 函数说明: 
	//************************************
	void CTreeNodeUI::SetItemFont(int font)
	{
		pItemButton->SetFont(font);
	}
	...
	// 函数说明: 
	//************************************
	int CTreeNodeUI::GetItemFont()
	{
		return pItemButton->GetFont();
	}
	...
	void CTreeNodeUI::SetAttribute( LPCTSTR pstrName, LPCTSTR pstrValue )
	{
		if(_tcscmp(pstrName, _T("text")) == 0 )
			...
			pItemButton->SetAttribute(_T("align"), pstrValue);
		else if(_tcscmp(pstrName, _T("textpadding")) == 0)
			pItemButton->SetAttribute(_T("textpadding"), pstrValue);
	...
	// 函数说明: 
	//************************************
	void CTreeViewUI::SetItemFont(int font)
	{
		for(int nIndex = 0;nIndex < GetCount();nIndex++){
			CTreeNodeUI* pTreeNode = static_cast<CTreeNodeUI*>(GetItemAt(nIndex));
			if(pTreeNode)
				pTreeNode->SetItemFont(font);
		}
	}
