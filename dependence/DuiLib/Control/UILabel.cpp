...
namespace DuiLib
{
	...
	void CLabelUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
		if( _tcscmp(pstrName, _T("align")) == 0 ) {
			...
			if( _tcsstr(pstrValue, _T("hcenter")) != NULL ) {
			...
			if( _tcsstr(pstrValue, _T("bcenter")) != NULL ) {
				m_uTextStyle &= ~(DT_TOP | DT_BOTTOM );			
		...
		if(!GetEnabledEffect())
		{
			...
			if( IsEnabled() ) {
				if( m_bShowHtml )
					...
					NULL, NULL, nLinks, m_uTextStyle&DT_WORDBREAK ? m_uTextStyle : DT_SINGLELINE | m_uTextStyle);
				else
					CRenderEngine::DrawText(hDC, m_pManager, rc, m_sText, m_dwTextColor, \
					m_iFont, m_uTextStyle&DT_WORDBREAK ? m_uTextStyle : DT_SINGLELINE | m_uTextStyle);
			}
			else {
				if( m_bShowHtml )
