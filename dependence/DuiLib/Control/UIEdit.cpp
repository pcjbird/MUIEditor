...
namespace DuiLib
{
	...
	void CEditWnd::Init(CEditUI* pOwner)
	{
		...
		m_pOwner->SetLastText(m_pOwner->GetText());
	...
	{
		m_pOwner->GetManager()->SendNotify(m_pOwner, DUI_MSGTYPE_TEXTCHANGED, (WPARAM)1);
	...
	void CEditUI::SetPos(RECT rc)
	{
		...
		if( m_pWindow != NULL ) {
			...
			if(bVisible)
			{
				...
				if(rcPos.left != invalidateRc.left || rcPos.right!= invalidateRc.right || rcPos.top != invalidateRc.top || rcPos.bottom != invalidateRc.bottom) bVisible = FALSE;
			}
			if(bVisible != ::IsWindowVisible(m_pWindow->GetHWND())) ::ShowWindow(m_pWindow->GetHWND(), bVisible);	
		}
	}
