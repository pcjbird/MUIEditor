	...
	while( ::PeekMessage(&msg, NULL, 0, 0,PM_REMOVE) ) {
		if( !CPaintManagerUI::TranslateMessage(&msg) ) {
			...
			try{
				...
				DUITRACE(_T("EXCEPTION: %s(%d)\n"), __FILET__, __LINE__);
#ifdef _DEBUG
				throw "CPaintManagerUI::OnUpdate";
#endif
			}
		}
		return true;
...
#pragma region IUnknown Implemnt
//
//	IUnknown::QueryInterface
//
HRESULT __stdcall CPaintManagerUI::QueryInterface (REFIID iid, void ** ppvObject)
{
	if(iid == IID_IDropTarget || iid == IID_IDropSource || iid == IID_IUnknown)
	{
		AddRef();
		*ppvObject = this;
		return S_OK;
...
//	IUnknown::AddRef
//
ULONG __stdcall CPaintManagerUI::AddRef(void)
{
...
//	IUnknown::Release
//
ULONG __stdcall CPaintManagerUI::Release(void)
{
...
#pragma region IDropSource Implement
//
...
//
HRESULT __stdcall CPaintManagerUI::GiveFeedback(DWORD dwEffect)
{
...
//
HRESULT __stdcall CPaintManagerUI::QueryContinueDrag(BOOL fEscapePressed, DWORD grfKeyState)
{
...
#pragma region IDropTarget Implemnt
//
//	QueryDataObject private helper routine
//
bool CPaintManagerUI::QueryDataObject(IDataObject *pDataObject)
{
	FORMATETC fmtetc = { CF_TEXT|CF_BITMAP, 0, DVASPECT_CONTENT, -1, TYMED_HGLOBAL };
...
//
DWORD CPaintManagerUI::DropEffect(DWORD grfKeyState, POINTL pt, DWORD dwAllowed)
{
	DWORD dwEffect = 0;
...
//
//
HRESULT __stdcall CPaintManagerUI::DragEnter(IDataObject * pDataObject, DWORD grfKeyState, POINTL pt, DWORD * pdwEffect)
{
	...
	if(m_fAllowDrop)
	{
		...
		*pdwEffect = DropEffect(grfKeyState, pt, *pdwEffect);
		...
		IDropTargetHelper *pDropTargetHelper = GetDropTargetHelper();
		if(NULL != pDropTargetHelper)
		{
			POINT _pt = {pt.x, pt.y};
			pDropTargetHelper->DragEnter(m_hWndPaint,pDataObject,&_pt,*pdwEffect);
		}
...
//
//
HRESULT __stdcall CPaintManagerUI::DragOver(DWORD grfKeyState, POINTL pt, DWORD * pdwEffect)
{
	if(m_fAllowDrop)
	{
		*pdwEffect = DropEffect(grfKeyState, pt, *pdwEffect);
		...
		{
			POINT _pt = {pt.x, pt.y};
			pDropTargetHelper->DragOver(&_pt,*pdwEffect);
		}
	}
	else
	{
...
//
HRESULT __stdcall CPaintManagerUI::DragLeave(void)
{
	IDropTargetHelper *pDropTargetHelper = GetDropTargetHelper();
	if(NULL != pDropTargetHelper)
	{
...
//
//
HRESULT __stdcall CPaintManagerUI::Drop(IDataObject * pDataObject, DWORD grfKeyState, POINTL pt, DWORD * pdwEffect)
{
	if(m_fAllowDrop)
	{
		POINT _pt = {pt.x, pt.y};
		CControlUI* pControl = FindControl(_pt);
		SendNotify(pControl, DUI_MSGTYPE_DROPDATA,(WPARAM)pDataObject, MAKELPARAM(pt.x,pt.y));
		*pdwEffect = DropEffect(grfKeyState, pt, *pdwEffect);
	}
	else
	{
	...
	{
		POINT _pt = {pt.x, pt.y};
		pDropTargetHelper->Drop(pDataObject, &_pt, *pdwEffect);
	}
...
void CPaintManagerUI::RegisterDropWindow(IDropTarget **ppDropTarget)
{
	// acquire a strong lock
	CoLockObjectExternal((IDropTarget*)this, TRUE, FALSE);

	// tell OLE that the window is a drop target
	RegisterDragDrop(m_hWndPaint, (IDropTarget*)this);

	*ppDropTarget = this;
}

void CPaintManagerUI::UnregisterDropWindow(IDropTarget *pDropTarget)
{
	if(NULL == pDropTarget)
	{
		return;
