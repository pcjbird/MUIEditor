...
bool CControlUI::IsSubControlOf(CControlUI* pParent)
{
...
HGLOBAL CControlUI::GetCopyText()
{
	HGLOBAL hMem;
	char   *ptr;
	
	CDuiString text = GetControlShareText();
	LPCTSTR data = text.GetData();
	size_t  len = text.GetLength();
	
	hMem = GlobalAlloc(GHND, sizeof(TCHAR)*(len+1));
	ptr  = (char *)GlobalLock(hMem);

	// copy the selected text and nul-terminate
	memcpy(ptr, (char *)data, sizeof(TCHAR)*len);
	ptr[sizeof(TCHAR)*len] = '\0';
...
BOOL CControlUI::CreateDragImage(SHDRAGIMAGE *psdi)
{
	if(!IsShareDragImage())
	{
		return FALSE;
	...
	psdi->sizeDragImage.cx = GetWidth();
	psdi->sizeDragImage.cy = GetHeight();
	psdi->ptOffset.x = GetWidth()/2;
	psdi->ptOffset.y = GetHeight()/2;
	psdi->crColorKey = CLR_NONE;
	HDC hdc = CreateCompatibleDC(NULL);
	if (hdc) 
	{
		psdi->hbmpDragImage = CreateBitmap(GetWidth(), GetHeight(), 1, 32, NULL);
		if (psdi->hbmpDragImage)
		{
			HBITMAP hbmPrev = SelectBitmap(hdc, psdi->hbmpDragImage);
			RECT rcPaint = {0,0,GetWidth(), GetHeight()};
			StretchBlt(hdc,0,0,GetWidth(),GetHeight(),m_pManager->GetPaintDC(),m_rcPaint.left,m_rcPaint.top,GetWidth(), GetHeight(),SRCCOPY);
...
HRESULT CControlUI::BeginDrag()
{
	...
	stgmed.hGlobal = GetCopyText();
	
	// Create IDataObject and IDropSource COM objects
	if(NULL != m_pManager)
	{
		m_pManager->GetDropSource(&pDropSource);
	}
	
	CreateDataObject(&fmtetc, &stgmed, 1, &pDataObject);

	IDragSourceHelper *pDragSrcHlp=(NULL != m_pManager) ? m_pManager->GetDragSourceHelper(): NULL;
	if(pDragSrcHlp)
	{
		SHDRAGIMAGE image;
		if(CreateDragImage(&image))
		{
...
void CControlUI::DoEvent(TEventUI& event)
{
	...
	if(event.Type == UIEVENT_DROPFILES)
	{
		m_pManager->SendNotify(this, DUI_MSGTYPE_DROPFILES, event.wParam, event.lParam);
		return;
	}
	if(event.Type == UIEVENT_BUTTONDOWN)
	{
		if(IsDropSource() && CanDragDrop())
		{
			m_bDropPrepared = TRUE;
		}
	}
