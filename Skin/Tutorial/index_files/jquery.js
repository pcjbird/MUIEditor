...
( function ( $, window, document, undefined ) {
	var	PREFIX =  "-" + getVendorPrefix().toLowerCase() + "-",
		...
		methods = {
			...
			scrollToDistance: function( distance, duration, easing, callback ) {
			...
			if (!isAnimating) {
				if (dir > 0) {
					$("nav .active").prev().find("> a").click();
									...
									if ($("nav .active").next().find("> a").length) {
	...
	function scrollToStep( stepParam, fromAnimation ) {
		...
		if (pathList[ stepParam ] ){
			...
			element.css( makeCSS( pathList[ stepParam ] ) , 2);
		}
	...
	function makeCSS( node ) {
		...
		$("body").css({"background-position-x": -centeredX/5, "background-position-y": -centeredY/5});


		//$("body").animate({backgroundPosition: '50px 700px'});
		// Only use transforms when page is rotated
		if ( normalizeAngle(node.rotate) === 0 ) {
			style.left = -centeredX;
			style.top = -centeredY;

			//$("html, body").scrollTop(-centeredY).scrollLeft(-centeredX);
